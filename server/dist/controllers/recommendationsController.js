...
const sequelize_1 = require("sequelize"); // Sequelize operators (Op) for complex queries (e.g., IN, NOT) and Sequelize for raw SQL functions
const LikedProduct_1 = __importDefault(require("../models/LikedProduct")); // Sequelize model for the LikedProducts table, tracking user likes
const ViewedProduct_1 = __importDefault(require("../models/ViewedProduct")); // Sequelize model for the ViewedProducts table, tracking product views
const SearchedProduct_1 = __importDefault(require("../models/SearchedProduct")); // Sequelize model for the SearchedProducts table, tracking search history
const ShoppingCart_1 = __importDefault(require("../models/ShoppingCart")); // Sequelize model for the ShoppingCart table, tracking cart items
const Product_1 = __importDefault(require("../models/Product")); // Sequelize model for the Products table, containing product details
const Vendor_1 = __importDefault(require("../models/Vendor")); // Sequelize model for the Vendors table, mapping users to vendor IDs
// ================================================
...
const getRecommendedProducts = async (req, res) => {
    try {
        ...
        const vendor = await Vendor_1.default.findOne({ where: { UserId } });
        ...
        const likedProducts = await LikedProduct_1.default.findAll({
            ...
            include: [
                {
                    model: Product_1.default, // Join with Products table to access CategoryId and VendorId
        ...
        const viewedProducts = await ViewedProduct_1.default.findAll({
            where: { UserId },
            include: [{ model: Product_1.default, attributes: ['id', 'CategoryId', 'VendorId'], required: false }],
            limit: 50,
            order: [['viewTimestamp', 'DESC']], // Sort by most recent views
        });
        // Query SearchedProducts, only including searches with a valid ProductId
        const searchedProducts = await SearchedProduct_1.default.findAll({
            where: { UserId, ProductId: { [sequelize_1.Op.ne]: null } }, // Exclude searches without a ProductId
            include: [{ model: Product_1.default, attributes: ['id', 'CategoryId', 'VendorId'], required: false }],
            limit: 50,
        ...
        const categoryScores = {}; // Maps CategoryId to a preference score
        const vendorScores = {}; // Maps VendorId to a preference score
        // Helper function to update category and vendor scores based on interactions
        const updateScores = (product, weight) => {
        ...
        if (topCategories.length < 3) {
            ...
            const popularCategories = await Product_1.default.findAll({
                attributes: ['CategoryId'], // Only fetch CategoryId
                group: ['CategoryId'], // Group by CategoryId
                order: [[sequelize_1.Sequelize.fn('COUNT', sequelize_1.Sequelize.col('id')), 'DESC']], // Sort by product count
                limit: 5, // Take top 5
            });
            // Extract CategoryIds, excluding those already in topCategories
            const additionalCategories = popularCategories
                .map((p) => p.CategoryId)
                .filter((id) => !!id && !topCategories.includes(id));
        ...
        if (topVendors.length < 3) {
            ...
            const popularVendors = await Product_1.default.findAll({
                attributes: ['VendorId'],
                group: ['VendorId'],
            ...
            const additionalVendors = popularVendors
                .map((p) => p.VendorId)
                .filter((id) => !!id && !topVendors.includes(id));
        ...
        const interactedProductIds = [
            ...new Set([
                ...
                ...searchedProducts.map((sp) => sp.ProductId).filter((id) => !!id), // Searched product IDs
        ...
        const availableProducts = await Product_1.default.count({
            where: {
                CategoryId: { [sequelize_1.Op.in]: topCategories }, // Must be in top categories
                ...(vendorId ? { VendorId: { [sequelize_1.Op.ne]: vendorId } } : {}), // Exclude vendor’s products if user is a vendor
            },
        });
        ...
        let recommendedProducts = await Product_1.default.findAll({
            where: {
                CategoryId: { [sequelize_1.Op.in]: topCategories }, // Must be in top categories
                id: { [sequelize_1.Op.notIn]: interactedProductIds }, // Exclude interacted products
                ...(vendorId ? { VendorId: { [sequelize_1.Op.ne]: vendorId } } : {}), // Exclude vendor’s own products
            },
            attributes: [
                'id', // Product ID
                'name', // Product name
        ...
        console.log('Recommended Product VendorIds:', recommendedProducts.map((p) => p.VendorId));
        ...
        if (recommendedProducts.length === 0 && interactedProductIds.length > 0) {
            ...
            recommendedProducts = await Product_1.default.findAll({
                where: {
                    CategoryId: { [sequelize_1.Op.in]: topCategories },
                    ...(vendorId ? { VendorId: { [sequelize_1.Op.ne]: vendorId } } : {}),
                },
                attributes: [
                    'id',
                    'name',
            ...
            console.log('Recommended Product VendorIds (No Exclusion):', recommendedProducts.map((p) => p.VendorId));
        }
        ...
        if (recommendedProducts.length === 0) {
            ...
            recommendedProducts = await Product_1.default.findAll({
                where: {
                    CategoryId: { [sequelize_1.Op.in]: topCategories },
                    id: { [sequelize_1.Op.notIn]: interactedProductIds },
                    ...(vendorId ? { VendorId: { [sequelize_1.Op.ne]: vendorId } } : {}),
                },
                attributes: [
                    'id',
            ...
            console.log('Recommended Product VendorIds (Categories Only):', recommendedProducts.map((p) => p.VendorId));
        }
        ...
        if (recommendedProducts.length === 0) {
            ...
            recommendedProducts = await Product_1.default.findAll({
                where: {
                    VendorId: { [sequelize_1.Op.in]: topVendors }, // Focus on preferred vendors
                    id: { [sequelize_1.Op.notIn]: interactedProductIds },
                    ...(vendorId ? { VendorId: { [sequelize_1.Op.ne]: vendorId } } : {}),
                },
                attributes: [
            ...
            console.log('Recommended Product VendorIds (Vendors Only):', recommendedProducts.map((p) => p.VendorId));
        }
        // Final fallback: Popular products
        if (recommendedProducts.length === 0) {
            console.log('Retrying with popular products');
            const popularProducts = await Product_1.default.findAll({
                where: {
                    ...(vendorId ? { VendorId: { [sequelize_1.Op.ne]: vendorId } } : {}), // Exclude vendor’s products
                },
                limit: 10,
            ...
            console.log('Popular Product VendorIds:', popularProducts.map((p) => p.VendorId));
