...
import { verify } from "jsonwebtoken";
import Message from "../models/Message";
import User from "../models/User"; // Ensure User model is imported for associations
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("sendMessage", async ({ receiverId, content }) => {
            ...
            try {
                ...
                const populatedMessage = await Message.findByPk(message.id, {
                    include: [
                        { model: User, as: "Sender", attributes: ["id", "businessName", "email"] },
                        { model: User, as: "Receiver", attributes: ["id", "businessName", "email"] },
                    ],
                });
